...
const teacherQuickActions = [
    {
        label: "Teaching load",
        icon: "📚",
        prompt: "Show assignments I created and submission counts by status",
    },
    {
        label: "My courses",
        icon: "🎓",
        prompt: "What courses am I assigned to teach?",
    },
    {
        label: "Announcements",
        icon: "📣",
        prompt: "What campus announcements apply to my department?",
    },
];
...
const adminQuickActions = [
    {
        label: "Announcements",
        icon: "📣",
        prompt: "List recent active campus announcements",
    },
    {
        label: "My profile",
        icon: "👤",
        prompt: "Summarize my staff profile from the portal",
    },
];
...
const superadminQuickActions = [
    {
        ...
        icon: "🏛️",
        prompt: "Give a brief overview of the university data you can help me query as superadmin",
    },
    {
        label: "Audit & safety",
        icon: "🔐",
        prompt: "What sensitive or destructive actions should I be careful about, and what is typically audited?",
    },
    {
        label: "Support escalations",
        icon: "⚡",
        prompt: "How can I use this assistant to help with org-wide or cross-department issues?",
    },
    {
        label: "Data scope",
        icon: "📊",
        prompt: "Explain what I can and cannot do here compared to students and staff",
    },
];
...
function formatChatMessageBody(raw: string): ReactNode {
    const re =
        /(https?:\/\/[^\s<]+[^\s<.,)]*|\/assignments\/attachment\/signed\?token=[^\s]+)/gi;
...
function MessageBubble({
    ...
    return (
        <div
            className={`flex ${isUser ? "justify-end" : "justify-start"} fade-in min-w-0`}
        >
            <div
            ...
            >
                ...
                >
                    <div className="text-white leading-relaxed">
                        {formatChatMessageBody(message.content)}
                    </div>
                    <p
                        className={`text-xs mt-2 flex items-center flex-wrap gap-2 ${isUser ? "text-white/60" : "text-gray-500"}`}
                    >
                        <span>
...
export default function ChatPage() {
    ...
    const [reportData, setReportData] = useState<StudentReportPayload | null>(
        null,
    );
    ...
    const isSuperadmin = roleKey === "superadmin" || roleKey === "superuser";
            ...
            const tryResumeSavedSession = async (): Promise<boolean> => {
                ...
                try {
                    ...
                    if (hist.messages?.length) {
                        const restored: ChatMessage[] = hist.messages.map(
                            (m, i) => ({
            ...
            >
                ...
                <div className="shrink-0 space-y-3 border-t border-white/10 p-4">
                    ...
                    <button
                        ...
                        className="flex w-full items-center gap-2 rounded-xl px-3 py-2.5 text-left text-sm text-gray-300 transition-all hover:bg-white/10 hover:text-white bg-red-500 cursor-pointer"
                    >
                        <svg
                            className="h-5 w-5 shrink-0 opacity-80"
                            fill="none"
                                ...
                                <div className="composer-toolbar">
                                    ...
                                    <button
                                        ...
                                        className={`composer-icon-btn flex items-center justify-center ${
                                            isListening
                                                ? "composer-icon-btn--voice-active"
                                        ...
                                        aria-label={
                                            isListening
                                                ? "Finish voice input (keep text in field)"
                                                : "Start voice input"
                                        }
                                    >
                                        {isListening ? (
